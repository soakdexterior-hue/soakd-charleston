...
export default function Home() {
  return (
    <>
      ...
      {/* ── HERO ── */}
      <section className="relative bg-gradient-to-b from-slate-100 to-slate-200 overflow-hidden" style={{ minHeight: '100vh' }}>
        {/* Background video */}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 py-16 md:py-24 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          {/* Left */}
          <div>
            <div className="inline-flex items-center gap-2 bg-emerald-100 border border-emerald-400 text-emerald-700 text-xs font-semibold px-4 py-1.5 rounded-full mb-6">
              <span className="w-2 h-2 bg-emerald-600 rounded-full" />
              Serving Charleston &amp; Surrounding Areas
            </div>
            <h1 className="text-5xl md:text-6xl font-extrabold text-slate-900 leading-[1.05] mb-4">
              Exterior<br />Cleaning.<br /><span className="text-emerald-600">Done Premium.</span>
            </h1>
            <p className="text-slate-700 text-base md:text-lg leading-relaxed mb-8 max-w-md">
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-10">
              <Link to="/contact">
                <Button size="lg" className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold px-8 py-6 text-base rounded-xl w-full sm:w-auto">
                  <Phone className="w-5 h-5 mr-2" />
                  Get a Free Quote
                </Button>
              </Link>
              <a href="tel:843-826-6708">
                <Button size="lg" className="bg-transparent hover:bg-slate-200 text-slate-900 font-bold px-8 py-6 text-base rounded-xl w-full sm:w-auto border-2 border-slate-400">
                  Call Now
                </Button>
              </a>
            </div>
          ...
          <div className="bg-white backdrop-blur-sm border border-slate-300 rounded-2xl p-6 shadow-lg">
            <p className="text-slate-600 text-xs font-semibold tracking-widest uppercase mb-1">Our Services</p>
            <p className="text-slate-500 text-xs mb-5">Residential &amp; Commercial</p>
      ...
      <section className="bg-slate-200 py-6 border-t border-slate-300 overflow-hidden">
        <div className="relative">
          <div className="absolute left-0 top-0 bottom-0 w-24 z-10" style={{ background: 'linear-gradient(to right, #e2e8f0, transparent)' }} />
          <div className="absolute right-0 top-0 bottom-0 w-24 z-10" style={{ background: 'linear-gradient(to left, #e2e8f0, transparent)' }} />
          <div className="flex gap-6 w-max" style={{ animation: 'ticker 40s linear infinite' }}>
            {[...REVIEWS, ...REVIEWS, ...REVIEWS, ...REVIEWS].map((r, i) => (
              <div key={i} className="flex items-center gap-3 bg-white border border-slate-300 rounded-xl px-5 py-3 shrink-0 shadow-sm">
                <div className="flex gap-0.5">{[1,2,3,4,5].map(s => <Star key={s} className="w-3 h-3 fill-yellow-400 text-yellow-400" />)}</div>
                <p className="text-slate-700 text-sm shrink-0 max-w-xs" style={{ maxWidth: '280px', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>"{r.text}"</p>
                <p className="text-slate-500 text-xs shrink-0">— {r.name}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="bg-slate-100 relative overflow-hidden">
        <div className="absolute left-0 top-0 bottom-0 w-20 z-10" style={{ background: 'linear-gradient(to right, #f1f5f9, transparent)' }} />
        <div className="absolute right-0 top-0 bottom-0 w-20 z-10" style={{ background: 'linear-gradient(to left, #f1f5f9, transparent)' }} />
      ...
      {/* ── EVERYTHING WE DO ── */}
      <section className="bg-slate-50 py-20 border-t border-slate-300">
        <div className="max-w-5xl mx-auto px-6">
          <p className="text-slate-600 text-xs font-semibold tracking-widest uppercase mb-2">Everything We Do</p>
          <h2 className="text-3xl font-extrabold text-slate-900 mb-10">Our Services</h2>
          <StaggerGrid className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {SERVICES.map(s => (
              <Link to={s.path} key={s.label} className="bg-white hover:bg-slate-100 border border-slate-300 rounded-2xl p-6 transition-all group block shadow-sm">
                <s.icon className="w-6 h-6 text-emerald-600 mb-4" />
                <h3 className="text-slate-900 font-bold text-base mb-1">{s.label}</h3>
      ...
      {/* ── VIDEO STRIP 2 ── */}
      <section className="bg-slate-100 py-10 border-t border-slate-300">
        <div className="max-w-5xl mx-auto px-6">
      ...
      <section className="bg-slate-50 py-20 border-t border-slate-300">
        <div className="max-w-5xl mx-auto px-6">
          ...
          <StaggerGrid className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
            {HOW_IT_WORKS.map(s => (
              <div key={s.title} className="bg-white border border-slate-300 rounded-2xl p-6 shadow-sm">
                <div className="w-8 h-8 rounded-lg bg-emerald-600 flex items-center justify-center text-white font-extrabold text-sm mb-4">{s.step}</div>
      ...
      {/* ── PHOTO + WHY US split ── */}
      <section className="bg-slate-100 border-t border-slate-300">
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="py-16 px-8 md:px-12">
            <p className="text-slate-600 text-xs font-semibold tracking-widest uppercase mb-2">Why Choose Soakd</p>
            <h2 className="text-3xl font-extrabold text-slate-900 mb-8">Charleston's Most Trusted Exterior Cleaners</h2>
            <div className="space-y-5">
              {WHY_US.map(w => (
                <div key={w.title} className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-slate-900 font-bold text-sm mb-0.5">{w.title}</h3>
      ...
      <section className="bg-slate-50 py-20 border-t border-slate-300">
        <div className="max-w-5xl mx-auto px-6">
          <p className="text-slate-600 text-xs font-semibold tracking-widest uppercase mb-2">What Customers Say</p>
          <h2 className="text-3xl font-extrabold text-slate-900 mb-10">Real Reviews from Real Neighbors</h2>
          <StaggerGrid className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {REVIEWS.map(r => (
              <div key={r.name} className="bg-white border border-slate-300 rounded-2xl p-6 shadow-sm">
                ...
                <p className="text-slate-500 text-xs">— {r.name}, {r.location}</p>
              </div>
            ))}
          </StaggerGrid>
      ...
      <section className="bg-slate-100 py-20 border-t border-slate-300">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <h2 className="text-4xl md:text-5xl font-extrabold text-slate-900 mb-3">Ready for a Cleaner Home?</h2>
          <p className="text-slate-600 mb-3">Most jobs booked within 48 hours. Free, no-obligation quotes.</p>
          <div className="flex flex-wrap items-center justify-center gap-4 text-slate-600 text-xs mb-10">
            {['Free Quotes','Fully Insured','Satisfaction Guaranteed','Local Business'].map(t => (
              <span key={t} className="flex items-center gap-1"><CheckCircle className="w-3 h-3 text-emerald-600" /> {t}</span>
            ))}
          </div>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <a href="tel:843-826-6708">
              <Button size="lg" className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold px-10 py-6 text-base rounded-xl w-full sm:w-auto">
                <Phone className="w-5 h-5 mr-2" />
                843-826-6708
              </Button>
            </a>
            <Link to="/contact">
              <Button size="lg" className="bg-transparent hover:bg-slate-200 text-slate-900 font-bold px-10 py-6 text-base rounded-xl w-full sm:w-auto border-2 border-slate-400">
